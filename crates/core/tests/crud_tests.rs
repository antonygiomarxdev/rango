...
fn test_insert_and_find_one() {
    ...
    let id = engine.insert_one(&coll, doc! { "name": "Alice", "age": 30 }).unwrap();
...
fn test_insert_preserves_existing_id() {
    ...
    let id = engine.insert_one(&coll, doc! { "_id": existing_id, "name": "Charlie" }).unwrap();
...
fn test_find_many() {
    ...
    engine.insert_one(&coll, doc! { "name": "Charlie" }).unwrap();
...
fn test_update_one_set() {
    ...
    let id = engine.insert_one(&coll, doc! { "name": "Alice", "age": 30 }).unwrap();
    let updated = engine.update_one(&coll, &id, doc! { "$set": { "age": 31 } }).unwrap();
...
fn test_update_one_unset() {
    ...
    let id = engine.insert_one(&coll, doc! { "name": "Alice", "age": 30 }).unwrap();
    let updated = engine.update_one(&coll, &id, doc! { "$unset": { "age": "" } }).unwrap();
...
fn test_update_one_inc() {
    ...
    let id = engine.insert_one(&coll, doc! { "name": "Alice", "score": 100i64 }).unwrap();
    let updated = engine.update_one(&coll, &id, doc! { "$inc": { "score": 5i64 } }).unwrap();
...
fn test_update_one_not_found() {
    ...
    let id = DocumentId::new_uuid_v7();
    let updated = engine.update_one(&coll, &id, doc! { "$set": { "x": 1 } }).unwrap();
...
fn test_large_collection_streaming() {
    ...
    for i in 0..10_000 {
        engine.insert_one(&coll, doc! { "index": i as i64 }).unwrap();
    }
