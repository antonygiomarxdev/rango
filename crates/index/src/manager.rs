...
use rango_types::{CollectionName, DocumentId, IndexDefinition, RangoError};
...
impl IndexManager {
    ...
    pub fn drop_index(&self, collection: &CollectionName, name: &str) -> Result<bool, RangoError> {
        let mut indexes = self
            ...
            .map_err(|e| RangoError::Storage(e.to_string()))?;
        Ok(indexes
            .remove(&(collection.0.clone(), name.to_string()))
            .is_some())
    }
    ...
    pub fn find_by_index(
        ...
        let tree = indexes
            .get(&(collection.0.clone(), field.to_string()))
            .ok_or_else(|| {
                RangoError::InvalidQueryOperator(format!("No index on field: {}", field))
            })?;
