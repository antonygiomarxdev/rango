...
struct KeepLastCandidateFilter;

impl BoundedContextFilterHook for KeepLastCandidateFilter {
    fn apply(&self, _request: &ReadRequest, candidates: Vec<Document>) -> Vec<Document> {
        candidates.into_iter().rev().take(1).collect()
    }
}
...
async fn pull_filter_selection_uses_stable_identity_for_duplicate_payloads() {
    ...
    let state = ServerState {
        ...
        control_plane: Arc::new(ControlPlane::with_hooks(
            ...
            Arc::new(KeepLastCandidateFilter),
    ...
    oplog
        .append(OplogEntry {
            seq: 0,
            timestamp: bson::DateTime::now(),
            mutation: make_mutation("w-first"),
    ...
    oplog
        .append(OplogEntry {
            seq: 0,
            timestamp: bson::DateTime::now(),
            mutation: make_mutation("w-second"),
            origin: OplogOrigin::Remote,
            applied: false,
            snapshot_anchor: None,
        })
    ...
    assert_eq!(response.mutations[0].write_id, "w-second");
