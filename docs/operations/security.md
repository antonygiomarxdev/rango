...
- `PushRequest` and `PullRequest` require `tenant_id` and `namespace`.
- Server ingress rejects mismatched token principal (`node_id`, `tenant_id`).
- Each mutation is validated for tenant+namespace consistency before append.
- Write decisions pass through control-plane hooks and produce auditable outcomes (`allow/reject/reason`).
- Default trust policy rejects low-trust candidates (`trust_score < 0.25`).
- Pull responses are filtered by tenant+namespace and include audit outcomes.
- Idempotent dedup is scoped to `(tenant_id, namespace, write_id)`.

### Audit Decision Contract

Every control-plane decision point emits one of:

- `allow`
- `sanitize`
- `reject`

With a machine-readable reason string (for example: `node_mismatch`, `tenant_mismatch`, `cross_tenant_or_namespace_mutation`, `trust_score_below_threshold:*`).

### Verification Coverage

- `cargo test -p rango-core --test sync_tests`
